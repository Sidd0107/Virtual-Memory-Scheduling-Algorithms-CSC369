...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
        ...
        //Find if dirty copy(If yes we need swapout).
        if(coremap[frame].pte->frame & PG_DIRTY){
            //Sets onswap if not set
            ...
            //swaps out
            int result=swap_pageout(frame, coremap[frame].pte->swap_off);
            if(result==INVALID_SWAP){
                printf("Error Swapping Page Out. Invalid Swap");
                exit(EXIT_FAILURE);
            }
            if(coremap[frame].pte->swap_off == INVALID_SWAP){
                coremap[frame].pte->swap_off = result;
            }
            ...
            // If clean and invalid swap
            if((coremap[frame].pte->swap_off)==INVALID_SWAP){
                //increments evict clean count
                evict_dirty_count++;
                //sets PG_ONSWAP
                coremap[frame].pte->frame=coremap[frame].pte->frame | PG_ONSWAP;
                coremap[frame].pte->frame = (coremap[frame].pte->frame & ~PG_VALID);
        ...
        // Check if its on swap the load from swap otherwise initialize frame(cant do cause of vaddr)
        if(p->frame & PG_ONSWAP){
            int result = swap_pagein(frame, p->swap_off);
            if(result!=0){
                printf("Error Swapping Page in. Invalid Swap");
                exit(EXIT_FAILURE);
            }
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    //Checks if pde is invalid then initializes it.
    if(((pgdir[idx].pde)&~PG_VALID)==0){
        pgdir[idx] = init_second_level();
    }
    
	// Use vaddr to get index into 2nd-level page table and initialize 'p'
    unsigned idxpgtbl = PGTBL_INDEX(vaddr);
    pgtbl_entry_t *entry= (pgtbl_entry_t *)((pgdir[idx].pde)&~PG_VALID);
    p = &(entry[idxpgtbl]);
	// Check if p is valid or not, on swap or not, and handle appropriately
    if((p->frame & PG_VALID)==0){
        miss_count++;
        //invalid p so frame must be allocated
        int frame = allocate_frame(p);
        //Note:ONSWAP has been set in allocate_frame.
        // Check if swap_off is invalid
        if(p->swap_off == INVALID_SWAP){
            //Page is not onswap implying page is not on disk.
